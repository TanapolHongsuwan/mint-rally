...
Autotask is an API that makes requests from the frontend. It connects to the Relayer.

![create autotask](documentImages/defender3.png)

Set any name for Name, Webhook for Trigger, and connect to the Relayer you created in 1.2. **No need to update the Code now**

![setup autotask](documentImages/defender4.png)

#### Get Autotask ID

Once the Autotask is created, go to the details page of Autotask and copy the unique ID at the end of the URL to your notes, you will use it later.
If the URL is something like `https://defender.openzeppelin.com/#/autotask/14645a02-f98e-0000-a659-000000000`, copy `14645a02-f98e-0000-a659-000000000`.

### 1.4 Create Defender Team API Key

Issue a Defender Team API key to update the Autotask code from the CLI. **Do not publish this API key on Github or similar platforms.**

Access [TeamAPIKeys](https://defender.openzeppelin.com/#/api-keys) and generate keys by selecting ManageRelayers and ManageAutotasks.
Once the key is generated, copy the API Key and Secret Key to your notes.
...
| Key | Value |
| --- | --- |
| TEAM_API_KEY | The API KEY obtained in 1.4 |
| TEAM_API_SECRET | The SECRET KEY obtained in 1.4 |
| MUMBAI_AUTOTASK_ID | The Autotask ID obtained in 1.3. For Mainnet deployment, set to `POLYGON_AUTOTASK_ID`. |
